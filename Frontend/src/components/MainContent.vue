...
watch(videoUrl, (newUrl) => {
  ...
  urlCheckTimeout = setTimeout(async () => {
    ...
    try {
      ...
      requestAnimationFrame(() => {
        resolutionOptions.value.forEach(option => {
          if (option.value !== 'all') {
  ...
  }, 300) // Debounce URL checks
})

// Optimize resolution change handler
...
<template>
  <div class="flex-1 overflow-auto w-full md:w-[calc(100%-16rem)] transition-all duration-200 ease-in-out">
    <div class="grid md:grid-cols-2 gap-4 p-4 md:gap-6 md:p-6">
      ...
      <div 
        class="bg-vscode-panel border border-vscode-border rounded-lg p-4 md:p-6 flex flex-col transition-all duration-300 ease-out"
      ...
      <!-- Preview Panel -->
      <div 
        class="bg-vscode-panel border border-vscode-border rounded-lg p-4 md:p-6 relative flex flex-col transition-all duration-300 ease-out"
